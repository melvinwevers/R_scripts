...
ggplot() +
  #geom_line(data = american_cigarette, aes(color ="Absolute Frequency", x = american_cigarette$year, y = american_cigarette$absolute)) +
  #geom_smooth(data = american_cigarette, aes(color ="Absolute Frequency", x = american_cigarette$year, y = american_cigarette$absolute)) +
  geom_bar() +
  geom_line(data = american_cigarette, aes(x = american_cigarette$year, y = american_cigarette$relative)) +
  geom_smooth(data = american_cigarette, aes(x = american_cigarette$year, y = american_cigarette$relative)) +
...
ggsave("CorpusBrands_abs_rel_ads.pdf", width=10, height=5)
...
ggplot(data=mild2, aes(x=year, y=value, group = corpus, colour = corpus)) +
  geom_line() +
  theme_bw() +
  theme(axis.text.x=element_text(angle = -45, hjust = 0)) +
  geom_point(size=1, shape=1, na.rm=TRUE) +
  #scale_x_continuous(breaks=pretty_breaks(n=10), limit = c(1950,1978)) +
  scale_y_continuous(breaks=pretty_breaks(n=10)) +
  xlab("Period")+ylab("Relative Frequency of Mild* per 1000 Cigarette Advertisements")

ggsave("mild_cigarette.pdf", width=10, height=5)
